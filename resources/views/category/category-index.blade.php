  ...
  <ol class="list-none inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      <div class="flex items-center">
        ...
        <a href="{{ route("categories.index")}}" class="ml-1 text-sm font-medium text-slate-700 hover:text-blue-600 md:ml-2 dark:text-slate-400 dark:hover:text-white">Daftar Kategori</a>
      </div>
    </li>
...
<div class="mb-8">
<h1 class="text-2xl font-medium">Tabel Daftar Kategori</h1>
<a href="{{ route("categories.create")}}" class="mt-1 w-max text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
<form class="w-full" method="GET" action="{{ route("categories.index")}}">   
...
<!-- Dropdown menu -->
<div id="dropdownDotsHorizontal-{{$category->id}}" class="z-10 hidden bg-white divide-y divide-slate-100 rounded-lg shadow w-44 dark:bg-slate-700 dark:divide-slate-600 border border-e-slate-300 dark:border-slate-800">
    <ul class="list-none py-2 text-sm text-slate-700 dark:text-slate-200" aria-labelledby="dropdownMenuIconHorizontalButton-{{$category->id}}">
          <li>
        <a href="{{ route("categories.show", $category)}}" class="block px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<span>Detail</span></a>
      ...
      <li>
        <a href="{{ route("categories.edit", $category)}}" class="block px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<span>Edit</span></a>
      ...
      <li>
                                    ...
                                    <button type="button"  data-modal-target="popup-modal-{{$category->id}}" data-modal-toggle="popup-modal-{{$category->id}}" class="w-full block px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<span>Hapus</span></button>
    ...
    <div class="relative w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal-{{$category->id}}">
            ...
            <div class="p-6 text-center">
                ...
                <h3 class="mb-3 text-lg font-normal text-gray-900 dark:text-gray-100">Apakah anda yakin ingin menghapus Kategori {{$category->name}}?</h3>
