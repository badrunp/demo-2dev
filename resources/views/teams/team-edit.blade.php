  ...
  <ol class="inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      <div class="flex items-center">
        ...
        <a href="{{ route("teams.update", $team)}}" class="ml-1 text-sm font-medium text-slate-700 hover:text-blue-600 md:ml-2 dark:text-slate-400 dark:hover:text-white">Edit</a>
      </div>
    </li>
  </ol>
</nav>
...
<div class="mb-8">
<h1 class="mb-2 text-2xl font-medium">Edit Daftar Tim</h1>
</div>

    <form class="mb-8" method="POST" action="{{ route('teams.update', $team) }}">
        @csrf
        @method("patch")
      <div class="mb-4">
         ...
         <option value="" {{!old("status", $team->status) ? "selected" : ""}}>Pilih status tim</option>
         <option {{ old("status",$team->status) == "active" ? "selected" : ""}} value="active">Aktif</option>
          <option {{ old("status",$team->status) == "noactive" ? "selected" : ""}} value="noactive">Tidak Aktif</option>
         </select>
        ...
        <div class="mb-4">
         ...
         <select id="user_id" name="user_id" class="border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-blue-500 dark:focus:border-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 rounded-md shadow-sm block w-full p-2.5" >
          ...
          <option value="{{$team->user->id}}" selected>{{$team->user->name}}</option>
         ...
         <x-input-error :messages="$errors->get('user_id')" class="mt-2" />
          ...
          <div class="mb-4">
            ...
            <x-text-input id="email" class="block mt-1 w-full" type="text" name="email" :value="old('email', $team->user->email)" placeholder="Terisi otomatis" readOnly/>
            <x-input-error :messages="$errors->get('email')" class="mt-2" />
        </div>
        
                  <div class="mb-4">
            <x-input-label for="jabatan" :value="__('Jabatan')" />
            <x-text-input id="jabatan" class="block mt-1 w-full" type="text" name="jabatan" :value="old('jabatan', $team->user->jabatan ? $team->user->jabatan : '-')" placeholder="Terisi otomatis" readOnly/>
            <x-input-error :messages="$errors->get('jabatan')" class="mt-2" />
        </div>
        
                          <div class="mb-4">
            <x-input-label for="phone" :value="__('Nomor HP')" />
            <x-text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone', $team->user->phone ? $team->user->phone : '-')" placeholder="Terisi otomatis" readOnly/>
            <x-input-error :messages="$errors->get('phone')" class="mt-2" />
        </div>
        
                                  <div class="mb-4">
            <x-input-label for="created_at" :value="__('Bergabung Pada')" />
            <x-text-input id="created_at" class="block mt-1 w-full" type="text" name="created_at" :value="old('created_at', $team->user->created_at)" placeholder="Terisi otomatis" readOnly/>
