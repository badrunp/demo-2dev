  ...
  <ol class="inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      <div class="flex items-center">
        ...
        <a href="{{ route("projects.index")}}" class="ml-1 text-sm font-medium text-slate-700 hover:text-blue-600 md:ml-2 dark:text-slate-400 dark:hover:text-white">Edit Projek</a>
      </div>
    </li>
    <li>
      <div class="flex items-center">
        ...
        <a href="{{ route("projects.edit",$project)}}" class="ml-1 text-sm font-medium text-slate-700 hover:text-blue-600 md:ml-2 dark:text-slate-400 dark:hover:text-white">Edit</a>
      </div>
    </li>
  </ol>
</nav>
...
<div class="mb-8">
<h1 class="mb-2 text-2xl font-medium">Edit Projek {{$project->name}}</h1>
</div>

    <form class="mb-8" method="POST" action="{{ route('projects.update', $project) }}" enctype="multipart/form-data">
        @csrf
        @method("patch")
        ...
        <div class="mb-4">
            ...
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                ...
                <div class="mb-4">
        <img src="{{asset('storage/' . $project->thumbnail)}}" alt="{{$project->name}}" class="w-44 mb-2"/>
...
<input class="block w-full text-sm text-slate-900 border border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-blue-500 dark:focus:border-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 rounded-md shadow-sm dark:placeholder-slate-400" aria-describedby="file_input_help" id="thumbnail" type="file" name="thumbnail">
         ...
         <option value="" {{!old("type_id", $project->type_id) ? "selected" : ""}}>Pilih Kategori</option>
         @foreach($types as $type)
         <option {{ old("type_id", $project->type_id) == $type->id ? "selected" : ""}} value="{{$type->id}}">{{$type->name}}</option>
         @endforeach
         </select>
         ...
         <option value="{{$tool->id}}" {{$project->tools->pluck("id")->contains($tool->id) ? 'selected' : ''}}> {{$tool->name}}</option>
         @endforeach
         </select>
         <x-input-error :messages="$errors->get('tools_id')" />
            ...
            <x-text-input id="demo_url" class="block mt-1 w-full" type="text" name="demo_url" :value="old('demo_url', $project->demo_url)" autofocus/>
         ...
         <option value="" {{!old("status", $project->status ? 'true' : 'false') ? "selected" : ""}}>Pilih Status</option>
         <option {{ old("status", $project->status ? 'true' : 'false') == "true" ? "selected" : ""}} value="true">Aktif</option>
          <option {{ old("status", $project->status ? 'true' : 'false') == "false" ? "selected" : ""}} value="false">Tidak Aktif</option>
         </select>
