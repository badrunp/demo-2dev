  ...
  <ol class="list-none inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      <div class="flex items-center">
        ...
        <a href="{{ route("faqs.edit",  $faq)}}" class="ml-1 text-sm font-medium text-slate-700 hover:text-blue-600 md:ml-2 dark:text-slate-400 dark:hover:text-white">Edit</a>
      </div>
    </li>
  </ol>
</nav>
...
<div class="mb-8">
<h1 class="mb-2 text-2xl font-medium">Edit Daftar Faq {{$faq->name}}</h1>
</div>

    <form class="mb-8" method="POST" action="{{ route('faqs.update', $faq) }}" enctype="multipart/form-data">
        @csrf
                 @method('patch')
