  ...
  <ol class="list-none inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      ...
      <div class="flex items-center">
        ...
        <a href="{{ route("faqs.show", $faq)}}" class="ml-1 text-sm font-medium text-slate-700 hover:text-blue-600 md:ml-2 dark:text-slate-400 dark:hover:text-white">Detail</a>
      </div>
    </li>
...
<div class="mb-8">
<h1 class="text-2xl font-medium">Detail Faq {{$faq->name}}</h1>
</div>


<dl class="mb-6 max-w-md text-gray-900 divide-y divide-gray-200 dark:text-white dark:divide-gray-700">
    <div class="flex flex-col pb-3">
        <dt class="mb-1 text-gray-500 text-sm mt-3 dark:text-gray-400">Pertanyaan</dt>
        <dd class="text-base font-semibold">{{$faq->pertanyaan}}</dd>
    </div>
        <div class="flex flex-col pb-3">
        ...
        <dd class="text-base font-semibold">{{$faq->created_at ? $faq->created_at : "-"}}</dd>
    </div>
</dl>

        <a href="{{route("faqs.index")}}" class="w-max focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-full text-sm px-5 py-2.5 dark:focus:ring-yellow-900 flex justify-items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
