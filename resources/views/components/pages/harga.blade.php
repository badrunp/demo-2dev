@props(["data"])
     ...
     <div class="flex flex-nowrap items-center xl:justify-center gap-4 overflow-x-auto snap-x px-4 pb-4">
              ...
              @endphp
                  ...
                  <div class="mb-4 flex items-center justify-center">
                   <p class="text-base py-1.5 px-6 text-slate-100 rounded-full bg-cyan-600 uppercase flex items-center gap-1"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
 ...
 <span>Paling Laris!</span></p>
                  ...
                  @endif
                     ...
                     <h5 class="mb-4 text-xl font-semibold uppercase text-center">{{$item->packet->name}}</h5>
                     ...
                     <div class="flex items-center justify-center gap-6 mb-4 text-sm">
                      <p class=" px-2 py-1 rounded-full bg-blue-100 text-slate-900 text-xs font-medium">Diskon {{$item->discount}}%</p>
                     ...
                     <ul role="list" class="space-y-5 my-7">
                      ...
                      @foreach($item->features as $feature)
                        ...
                        @if($feature->desc)
                                                   <button data-tooltip-target="tooltip-right-{{$feature->id}}" data-tooltip-placement="right" type="button" ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
  <path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm11.378-3.917c-.89-.777-2.366-.777-3.255 0a.75.75 0 01-.988-1.129c1.454-1.272 3.776-1.272 5.23 0 1.513 1.324 1.513 3.518 0 4.842a3.75 3.75 0 01-.837.552c-.676.328-1.028.774-1.028 1.152v.75a.75.75 0 01-1.5 0v-.75c0-1.279 1.06-2.107 1.875-2.502.182-.088.351-.199.503-.331.83-.727.83-1.857 0-2.584zM12 18a.75.75 0 100-1.5.75.75 0 000 1.5z" clip-rule="evenodd" />
</svg>
</button>
<div id="tooltip-right-{{$feature->id}}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
    {{$feature->desc}}
