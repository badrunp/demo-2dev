  ...
  <ol class="list-none inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      <div class="flex items-center">
        ...
        <a href="{{ route("benefits.create")}}" class="ml-1 text-sm font-medium text-slate-700 hover:text-blue-600 md:ml-2 dark:text-slate-400 dark:hover:text-white">Tambah</a>
      </div>
    </li>
  </ol>
</nav>
...
<div class="mb-8">
<h1 class="mb-2 text-2xl font-medium">Tambah Daftar Benefit</h1>
</div>

    <form class="mb-8" method="POST" action="{{ route('benefits.store') }}" enctype="multipart/form-data">
        @csrf
...
<textarea id="icon" rows="4" name="icon" class="block p-2.5 w-full text-sm text-slate-900 border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-blue-500 dark:focus:border-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 rounded-md shadow-sm mb-2" placeholder="Paste icon disini...">{{old("icon")}}</textarea>
