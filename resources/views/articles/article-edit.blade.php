  ...
  <ol class="inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      <div class="flex items-center">
        ...
        <a href="{{ route("articles.edit", $article)}}" class="ml-1 text-sm font-medium text-slate-700 hover:text-blue-600 md:ml-2 dark:text-slate-400 dark:hover:text-white">Edit</a>
      </div>
    </li>
  </ol>
</nav>
...
<div class="mb-8">
<h1 class="mb-2 text-2xl font-medium">Edit Artikel {{$article->title}}</h1>
</div>

    <form class="mb-8" method="POST" action="{{ route('articles.update',$article) }}" enctype="multipart/form-data">
        @csrf
        @method("patch")
        ...
        <div class="mb-4">
            ...
            <x-input-error :messages="$errors->get('title')" class="mt-2" />
                ...
                <div class="mb-4">
                <img src="{{asset('storage/' . $article->thumbnail)}}" alt="{{$article->name}}" class="w-44 mb-2"/>
...
<input class="block w-full text-sm text-slate-900 border border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-blue-500 dark:focus:border-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 rounded-md shadow-sm dark:placeholder-slate-400" aria-describedby="file_input_help" id="thumbnail" type="file" name="thumbnail">
         ...
         <option value="" {{!old("category_id", $article->category_id) ? "selected" : ""}}>Pilih Kategori</option>
         ...
         <option value="" {{!old("status",$article->status ? 'true' : 'false' ) ? "selected" : ""}}>Pilih Status</option>
         <option {{ old("status", $article->status ? 'true' : 'false') == "true" ? "selected" : ""}} value="true">Published</option>
          <option {{ old("status", $article->status ? 'true' : 'false') == "false" ? "selected" : ""}} value="false">Unpublished</option>
         </select>
