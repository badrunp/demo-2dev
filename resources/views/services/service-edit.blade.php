...
<nav class="flex mb-8" aria-label="Breadcrumb">
  <ol class="list-none list-none inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      <div class="flex items-center">
        ...
        <a href="{{ route("services.edit",  $service)}}" class="ml-1 text-sm font-medium text-slate-700 hover:text-blue-600 md:ml-2 dark:text-slate-400 dark:hover:text-white">Edit</a>
      </div>
    </li>
  </ol>
</nav>
...
<div class="mb-8">
<h1 class="mb-2 text-2xl font-medium">Edit Layanan {{$service->name}}</h1>
</div>

    <form class="mb-8" method="POST" action="{{ route('services.update', $service) }}" enctype="multipart/form-data">
        @csrf
                 @method('patch')
        ...
        <div class="mb-4">
            ...
            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $service->name)" autofocus/>
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>
        
                <div class="mb-4">
        
<label class="block mb-2 text-sm font-medium text-slate-900 dark:text-white" for="photo">Pilih Gambar</label>
        <image src="{{asset("storage/" . $service->photo)}}" alt="{{$service->name}}" class="w-12 h-12 mb-2" />
        </dd>
...
<textarea id="message" rows="4" name="desc" class="block p-2.5 w-full text-sm text-slate-900 border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-blue-500 dark:focus:border-blue-600 focus:ring-blue-500 dark:focus:ring-blue-600 rounded-md shadow-sm" placeholder="Tulis deskripsi disini...">{{old("desc", $service->desc)}}</textarea>
            <x-input-error :messages="$errors->get('desc')" class="mt-2" />
       </div>
        
        <div class="flex items-center gap-2">
        <a href="{{route("services.index")}}" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-full text-sm px-5 py-2.5 dark:focus:ring-yellow-900 flex justify-items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
