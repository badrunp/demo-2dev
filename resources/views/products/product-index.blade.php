...
<div class="mb-8">
<h1 class="text-2xl font-medium">Tabel Daftar Produk</h1>
<a href="{{ route("products.create")}}" class="mt-1 w-max text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
<th scope="row" class="flex items-center px-6 py-4 text-slate-900 whitespace-nowrap dark:text-white">
                        ...
                        <p class="text-base font-semibold">{{$product->service->name}}</p>
...
<!-- Dropdown menu -->
<div id="dropdownDotsHorizontal-{{$product->id}}" class="z-10 hidden bg-white divide-y divide-slate-100 rounded-lg shadow w-44 dark:bg-slate-700 dark:divide-slate-600 border border-e-slate-300 dark:border-slate-800">
    <ul class="list-none py-2 text-sm text-slate-700 dark:text-slate-200" aria-labelledby="dropdownMenuIconHorizontalButton-{{$product->id}}">
          <li>
        <a href="{{ route("products.show", $product)}}" class="block px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<span>Detail</span></a>
      ...
      <li>
        <a href="{{ route("products.edit", $product)}}" class="block px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<span>Edit</span></a>
      ...
      <li>
                                    ...
                                    <button type="button"  data-modal-target="popup-modal-{{$product->id}}" data-modal-toggle="popup-modal-{{$product->id}}" class="w-full block px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<span>Hapus</span></button>
                                    ...
                                    <div id="popup-modal-{{$product->id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">

            <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal-{{$product->id}}">
            ...
            <div class="p-6 text-center">
                ...
                <h3 class="mb-3 text-lg font-normal text-gray-900 dark:text-gray-100">Apakah anda yakin ingin menghapus produk {{$product->service->name}}?</h3>
